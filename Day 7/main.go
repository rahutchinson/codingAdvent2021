...
func main() {
	...
	fmt.Println(checkUpAndDown(numbs, average, 200))
}


func checkEverything(numbs []int) (int, int) {
	var bestIndex = 0
	var bestGas = calculateGas(numbs, bestIndex)
	for i := 0; i < len(numbs); i++ {
		gas := calculateGas(numbs, numbs[i])
		fmt.Print(numbs[i])
		fmt.Print(" ")
		fmt.Println(gas)
		if gas < bestGas {
	...
	return numbs[bestIndex], bestGas
}

func checkUpAndDown(numbs []int, averageValue int, toMove int) (int, int) {
	var bestValue = averageValue
	var bestGas = calculateGas(numbs, averageValue)
	for i := 1; i < toMove; i++ {
		gas := calculateGas(numbs, averageValue-i)
		if gas < bestGas {
			bestValue = averageValue - i
			bestGas = gas
		}
		gas = calculateGas(numbs, averageValue+i)
		if gas < bestGas {
	...
	return bestValue, bestGas
}

func triangleNumCalculator(num int) int {
	return num*(num+1)/2
}


func calculateGas(numbs []int, moveToo int) int {
	var gas = 0
	var steps = 0
	for i, _ :=range numbs {
		steps = int(math.Abs(float64(numbs[i] - moveToo)))
		gas += triangleNumCalculator(steps)
	}
	return gas
}
